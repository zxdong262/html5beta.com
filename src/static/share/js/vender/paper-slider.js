...
(function ($) {
  function PS (opts, ob) {
    ...
    const th = this
    const defs = th.defs = $.extend(defaults, opts)
    const cssSet = {
      position: 'absolute',
      left: 0,
      right: 0,
    ...
    th.t.on('click', '.ps-link', function () {
      ...
      th.onAction = true
      const i1 = parseInt($(this).data('ps-page'))
      const i2 = (i1 + th.len) % th.len
      const isNext = i1 > th.currentPage
    ...
    if (defs.hasNav) {
      ...
      th.t.on('click', '.ps-nav', function () {
			  if (th.onAction) return
  			th.onAction = true
  			const isNext = $(this).hasClass('ps-nav-next')
  			const len = th.len
  			const i = isNext ? (th.currentPage + 1 + len) % len : (th.currentPage - 1 + len) % len
  			th.action(isNext, i)
      })
    }
  }
  ...
  PS.prototype = {
      ...
      const th = this
      const defs = th.defs
      const speed = defs.speed
      const c = th.currentPage
      const ps = th.ps
      const step = isNext ? 50 : -50
      const cp = ps.eq(c)
      const ip = ps.eq(index)
      cp.css({
    ...
    autoroll: function () {
      ...
      if (!t.onAction && !t.pause) {
				  t.onAction = true
				  const i = (t.currentPage + 1 + t.len) % t.len
