...
(function (global, factory) {
  ...
  function defineReactive (obj, key, val, doNotObserve) {
    ...
    let childOb = doNotObserve ? isObject(val) && val.__ob__ : observe(val)
    Object.defineProperty(obj, key, {
      enumerable: true,
      configurable: true,
  ...
  function checkComponentAttr (el, options) {
    ...
    if (!commonTagRE.test(tag) && !reservedTagRE.test(tag)) {
      if (resolveAsset(options, 'components', tag)) {
        ...
        const is = hasAttrs && getIsBinding(el)
        if (is) {
          return is
  ...
  const vFor = {
    ...
    getCachedFrag: function getCachedFrag (value, index, key) {
      ...
      let frag
      if (key || trackByKey || primitive) {
        const id = trackByKey ? trackByKey === '$index' ? index : value[trackByKey] : key || value
        frag = this.cache[id]
      } else {
        frag = value[this.id]
      }
  ...
  const checkbox = {
    ...
    bind: function bind () {
      ...
      this.listener = function () {
        const model = self._watcher.value
        if (isArray(model)) {
  ...
  const vClass = {
    ...
    handleObject: function handleObject (value) {
      this.cleanup(value)
      const keys = this.prevKeys = Object.keys(value)
      for (let i = 0, l = keys.length; i < l; i++) {
        const key = keys[i]
        if (value[key]) {
  ...
  function compile (el, options, partial) {
    ...
    const childLinkFn = !(nodeLinkFn && nodeLinkFn.terminal) && el.tagName !== 'SCRIPT' && el.hasChildNodes() ? compileNodeList(el.childNodes, options) : null
  ...
  function checkTerminalDirectives (el, options) {
    ...
    let value, dirName
    for (let i = 0, l = terminalDirectives.length; i < l; i++) {
  ...
  function stateMixin (Vue) {
    ...
    Vue.prototype._initData = function () {
      ...
      const data = this._data = dataFn ? dataFn() : {}
      const props = this._props
      const runtimeData = this._runtimeData ? typeof this._runtimeData === 'function' ? this._runtimeData() : this._runtimeData : null
  ...
  Directive.prototype._checkStatement = function () {
    ...
    if (expression && this.acceptStatement && !isSimplePath(expression)) {
      const fn = parseExpression(expression).get
  ...
  function miscMixin (Vue) {
    ...
    Vue.prototype._resolveComponent = function (id, cb) {
      const factory = resolveAsset(this.$options, 'components', id)
      if ('development' !== 'production') {
  ...
  function dataAPI (Vue) {
    ...
    Vue.prototype.$set = function (exp, val) {
      const res = parseExpression(exp, true)
      if (res && res.set) {
  ...
  const partial = {
    ...
    insert: function insert (id) {
      const partial = resolveAsset(this.vm.$options, 'partials', id)
      if ('development' !== 'production') {
  ...
  function filterBy (arr, search, delimiter) {
    ...
    const keys = toArray(arguments, n).reduce(function (prev, cur) {
